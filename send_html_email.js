...
// This is the old minified function, it had this line in it which I took out because it doesn't make sense:  e = e.replace(/rn/g, "n");
...
/*
 * Sends an HTML email
 * To construct an HTML email, I found the following links helpful:
...
function run(params) {  
  /*
 * A minified Base64 library. Note that it must use special characters "-_=" instead of "+/="
 * because GMail expects URL-encoded base64.
 */
  ...
  if (params.cc) {
    headers_obj['CC'] = params.cc
  }
  
  if (params.from) {
   	headers_obj['From'] = params.from 
    headers_obj['Sender'] = params.from
  }
  
  // Construct email to meet RFC standards
  var email = '';
  ...
  api.log(Base64.decode(Base64.encode(email)));
  
  //var replacedEmail = email.replace("-", "+").replace("_","/")
  // TODO can we get support for btoa()?
  //var base64Encoded = Window.btoa(replacedEmail)

  // Construct payload for GMail API
  var body = {
  ...
  // Send the email
  var results = api.run('google_mail.send_message', {
      "userId": "me",
